"""Macy's US parsers (conservative; fixture-tested)."""
...
_BASE_URL = "https://www.macys.com"
_SOURCE_ID = "macys_us"


class MacysDealsPageParser(BaseParser):
    parser_name = "macys_deals_page_parser"
    ...
    def parse(self, raw_content: str, source_url: str = "") -> ParserResult:
        ...
        blocks = find_promo_blocks(soup, extra_selector='[class*="card"], section')
        for block in blocks:
        ...
        if not blocks:
            result.errors.append("no deal blocks recognized")
        return result


class MacysSaleListingParser(BaseParser):
    parser_name = "macys_sale_listing_parser"
    parser_version = "0.1.0"
